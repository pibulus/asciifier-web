...
export class ThemeSystem {
  ...
  loadTheme(): Theme {
    ...
    if (saved) {
      try {
        ...
        const theme = this.config.themes.find(t => t.name === savedTheme.name);
        if (theme) {
          this.currentTheme = theme;
          return theme;
        }
  ...
  // Cycle to next theme
  cycleTheme(): Theme {
    const currentIndex = this.config.themes.findIndex(t => t.name === this.currentTheme.name);
...
export class RandomThemeGenerator {
  ...
  static generateHarmonicTheme(baseMode: "light" | "dark" = "light"): Theme {
    ...
    // Choose harmony type
    const harmonyTypes = ["analogous", "triadic", "complementary", "split-complementary"];
  ...
  static generateConstrainedRandom(
    ...
    const newSat = Math.max(0, Math.min(100,
      baseHsl.s + (Math.random() - 0.5) * variance));
    const newLight = Math.max(0, Math.min(100,
      baseHsl.l + (Math.random() - 0.5) * variance / 2));

    const isLight = baseTheme.base.includes("#F") || baseTheme.base.includes("gradient");
